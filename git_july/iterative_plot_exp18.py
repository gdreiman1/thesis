...
exp.log_code = True
exp.log_other('Hypothesis','''These are my plots for tuned with 5%, 5%, random,test''')
import pickle
import os
import pandas as pd
...
first8 = 'tuned_1_svmmod_3test0.pkl'
second8 = 'tuned_1_svmmod_3test1.pkl'
third8 = 'tuned_1_svmmod_3test2.pkl'
...
plot_avg_percent_found(pd.concat([expr_1,expr_2,expr_3]),'Mean Active Recovery for Test data',10,5)
g = sns.relplot(x="Iteration Number", y="Score", hue='Classifier',style="Metric", col="AID", col_wrap=3, data=pd.concat([expr_1,expr_2,expr_3]),kind='line',legend='full',markers= True )
