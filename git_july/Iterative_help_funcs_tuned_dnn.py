...
def train_SVM(X_train,X_test,y_train,y_test,base_X_train):
    sgd_linear_SVM = SGDClassifier(loss='hinge', penalty='l2', alpha=0.0001, l1_ratio=0.15, fit_intercept=True, max_iter=10000, 
                                                    tol=0.001, shuffle=True, verbose=0, epsilon=0.1, n_jobs=-1, random_state=None, learning_rate='optimal', eta0=0.0007, power_t=0.5, early_stopping=False, 
                                                    validation_fraction=0.1, n_iter_no_change=5, class_weight='balanced', warm_start=False, average=5)
...
def train_RF(X_train,X_test,y_train,y_test,base_X_train):
    ...
    rf = RandomForestClassifier(n_estimators=1200, class_weight="balanced", max_features='log2',bootstrap=True,min_samples_split = 8, min_samples_leaf = 3, n_jobs = -1)
...
def train_LGBM(X_train,X_test,y_train,y_test,base_X_train):
    #make model class
    lgbm_model = lgb.LGBMClassifier(boosting_type='dart', num_leaves=42, max_depth=-1, learning_rate=0.25, n_estimators=1200, subsample_for_bin=200000, 
                                    objective='binary', is_unbalance=False, max_bin=200, min_child_weight=0.001, min_child_samples=30, subsample=1.0, 
...
def train_DNN(X_train,X_test,y_train,y_test,base_X_train):
        ...
        train_len = len(base_X_train)
        len_test = len(y_test)
        num_epochs = int(20*(1+np.log(train_len/(0.1*len_test))))
