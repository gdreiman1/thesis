...
exp.log_code = True
exp.log_other('Hypothesis','''Taking 5% batches, 80% of batch is the top ranked compounds, remaining 20% is random selection for first 5
...
for repetition_number in range(3):
    metric_dict_list = []
    multi_dump_path = os.path.join('/home/gabriel/Dropbox/UCL/Thesis/Data/', 'ranked_random_run'+str(repetition_number)+'.pkl') 
    ...
    for AID in AID_list:
            ...
            with parallel_backend('multiprocessing'):
                start_index_metalist = Parallel(n_jobs=num_models)(delayed(getNextIterInds)(firstPicksList=i, fplist=j,bottom_to_select=k) for i,j,k in zip([[],[],[]],[fplist,fplist,fplist],[]))
