...
exp.log_code = True
exp.log_other('Hypothesis','''15% start 10% iter, random,svm with hinge loss''')
exper_file_name = 'tuned_5_svmmod_corrected'
        ...
        for rep_num in range(3):
            ...
            while iter_num < end_iter:
                ...
                for list_idx,[model_type,start_indexs] in enumerate(zip(model_list,start_ind_list)):
                    ...
                    if len(test_index) > int(0.1 *library_size):
                        ...
                        next_inds=[]
                        top_to_select = int(len(activity_table)*0.08)
                        explore_select = int(len(activity_table)*0.02)
