...
exp.log_code = True
exp.log_other('Hypothesis','Comparing diverse with smaller iter sizes to random with small itersize')
    ...
    while iter_num < 9:
        ...
        for list_idx,[model_type,start_indexs] in enumerate(zip(model_list,start_ind_list)):
            ...
            if num_pred_active >= int(len(activity_table)*0.08*iterRel2Start):
                ...
                bottom_to_select = int((len(activity_table)*0.1-len(next_inds))*0.5*iterRel2Start)
                next_inds=next_inds+neg_preds_df.head(bottom_to_select)['activity_table_index'].tolist()
        ...
        with parallel_backend('multiprocessing'):
            if selection_type == 'Diverse':
                start_ind_list = Parallel(n_jobs=4)(delayed(getNextIterInds)(firstPicksList=i, fplist=j,bottom_to_select=k) for i,j,k in zip(start_ind_list,fp_metalist,diverse_size_list))
            elif selection_type == 'Random':
                start_ind_list = Parallel(n_jobs=4)(delayed(getRandomIterInds)(firstPicksList=i, fplist=j,bottom_to_select=k) for i,j,k in zip(start_ind_list,fp_metalist,diverse_size_list))

        iter_num +=1
metrics_df = pd.DataFrame(metric_dict_list)
multi_dump_path = os.path.join('/home/gabriel/Dropbox/UCL/Thesis/Data/', 'first_diverse_9iter_run.pkl') 
