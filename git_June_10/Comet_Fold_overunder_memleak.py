...
#        from keras import backend as K
    ...
    def train_kSVM(X_train,X_test,y_train,y_test,split_ID):
        kSVM = SVC(kernel = 'rbf', degree = 3, gamma = 'auto', coef0 = 0.0, C = 1.0, tol = 0.001, probability = False, class_weight = 'balanced', shrinking = False, cache_size = None, verbose = False, max_iter = -1, n_jobs = -1, max_mem_size = -1, random_state = None, decision_function_shape = 'ovo')
...
#        fast_NN.add(Dropout(0.5))
        ...
        fast_NN.compile(loss = 'categorical_crossentropy', optimizer='adam',metrics=['categorical_accuracy'])
    ...
    classifier_dict = {'SVM': train_SVM, 'RF': train_RF, 'LGBM':train_LGBM,'DNN':train_DNN,'kSVM':train_kSVM}
...
#for AID in ['AID_605','AID_596','AID_893','AID_894']:
...
if 'win' in sys.platform:
    ...
    multi_dump_path = os.path.join('/home/gabriel/Dropbox/UCL/Thesis/Data/', 'multiple_metrics_df_over_under_kSVM.pkl') 
    multiple_metrics_df.to_pickle(multi_dump_path)