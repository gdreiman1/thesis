...
def comet_Fold(save_path,embedding_type, model_type, bin_labels):
    ...
    exp = Experiment(api_key="sqMrI9jc8kzJYobRXRuptF5Tj",
                            project_name="80_10_baseline", workspace="gdreiman1", disabled = True)
    exp.log_code = True
...
for AID in ['AID_1345083','AID_624255','AID_449739','AID_995','AID_938','AID_628','AID_605','AID_596','AID_893','AID_894']:
    ...
    if count == 0:
        multiple_metrics_df = comet_Fold(save_path, 'MFPMolChars','RF',True)
        count+=1
    else:
        multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MFPMolChars','RF',True)])
    multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MFP','RF',True)])
    multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MolChars','RF',True)])
#dump as pkl
    multiple_metrics_df.to_pickle('/media/Data/gabriel/Thesis/Data/multiple_metrics_df.pkl')
