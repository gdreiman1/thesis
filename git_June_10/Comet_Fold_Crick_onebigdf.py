# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 17:55:31 2019
...
#            exp.log_parameters(lgbm.get_params())
    ...
    def calc_and_save_metrics(X_train,X_test,y_train,y_test,split_index,model_type,
                              embedding_type,AID,metric_names,metric_dict_list,map_name,split_num,little_split_num):
        ...
        if little_split_num == 'NaN':
            ...
            split_size = '10%'
        metric_dict_list.append(dict(zip(metric_names,[model_type,embedding_type,AID,split_num,little_split_num,split_size,split_index,map_name]+results_array)))
    ...
    for split_num, [train_ind, test_ind] in enumerate(big_splitter.split(labels,labels)):
        ...
        metric_dict_list = calc_and_save_metrics(X_train,X_test,y_train,y_test,split_index,model_type,
                              embedding_type,AID,metric_names,metric_dict_list,map_name,split_num,little_split_num)
...
for AID in ['AID_1345083','AID_624255','AID_449739','AID_995','AID_938','AID_628','AID_605','AID_596','AID_893','AID_894']:
    try:  
        ...
        save_path = Path(AID_path+ '/' + AID +'_processed.pkl')
        if count == 0:
            multiple_metrics_df = comet_Fold(save_path, 'MFPMolChars','DNN',True)
            count+=1
        else:
            multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MFPMolChars','DNN',True)])
    except Exception as e:
        traceback.print_exc(file=log)
        continue
    try:
        multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MFP','DNN',True)])
    except Exception:
        traceback.print_exc(file=log)
        continue
    try:
        multiple_metrics_df = pd.concat([multiple_metrics_df,comet_Fold(save_path, 'MolChars','DNN',True)])
    except Exception:
        traceback.print_exc(file=log)
        continue
#dump as pkl
if 'win' in sys.platform:
    multi_dump_path = os.path.join(r'C:\Users\gdrei\Dropbox\UCL\Thesis\Data', 'multiple_metrics_df_DNN.pkl') 
    multiple_metrics_df.to_pickle(multi_dump_path)
else:
    multi_dump_path = os.path.join('/media/Data/gabriel/Thesis/Data/', 'multiple_metrics_df_DNN.pkl') 
    multiple_metrics_df.to_pickle(multi_dump_path)